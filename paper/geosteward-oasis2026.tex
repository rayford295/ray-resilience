...
debris volumes, social vulnerability, and reliability-gated cross-view
street imagery on H3 grids, delivered through an installable, keyless
web app with resident and planner modes. The test suite doubles as a
verifiable evaluation environment (310 tests in CI), and the audit
...
default-deny: the citable unit is the citation, never the prose.
...
closed instead of reporting zero fires.
...
\item \emph{Eaton Fire 2025 (CA):} 18{,}428 CAL FIRE DINS structure
  ...
  $\rightarrow$ a 109-cell evidence grid. A damage class with $n{=}30$
...
\item \emph{Hurricane Milton 2024 (FL):} 2{,}556 labeled pre/post
  ...
  A generated-imagery dataset was excluded, and the exclusion itself is
...
\item \emph{Hurricane Ian 2022 (FL):} 886 matched samples $\rightarrow$
  ...
  severity link exists --- candor over coverage.
\end{itemize}
All source imagery traces to a hashed dataset registry (134{,}272
files, $\sim$33\,GB, SHA-256).
...
266 Python tests and 44 app tests run in CI on every push, including a
