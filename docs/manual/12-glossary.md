...
|---|---|---|---|
| AOI | 关注区域（AOI） | The event's geographic boundary; exposure, vulnerability, and damage-assessment claims are authorized only inside it, and an address outside one gets Tier-1 watch only, stated as such rather than extrapolated. | `src/geosteward/harness/policy_v1.yaml` |
| artifact ID | 制品 ID | The first 12 hex characters of an artifact's manifest SHA-256, used as a citation tag (`[artifact:XXXXXXXXXXXX]`); every factual sentence the agent emits must carry one. | `src/geosteward/gateway/context.py` |
...
| CAL FIRE DINS | CAL FIRE DINS（损毁勘查） | Damage Inspection (DINS): a per-structure post-fire damage survey conducted by CAL FIRE, the California state fire and forestry agency. In this project the data is a disclosure risk, not a licensing one — it is public-domain but resolution-capped to `parcel` and never served to the public. | `src/geosteward/deepcase/dins.py` |
| CDC SVI | CDC SVI（社会脆弱性指数） | The Social Vulnerability Index published by the US Centers for Disease Control and Prevention / ATSDR: tract-level percentile ranks (2022 vintage in this project) describing a community's vulnerability to external stress. | `src/geosteward/deepcase/svi.py` |
| claim plane | 断言平面 | The half of `policy_v1.yaml` scoping what the agent may assert per request — by role, purpose, resolution, evidence tier, AOI membership, and verifiability. First-match-wins over ordered rules; unmatched requests are denied. | `src/geosteward/harness/policy.py` |
| declared unknowns | 声明未知项 | Gaps in the evidence stated explicitly, with the same prominence as findings, rather than omitted or smoothed over — an event with only Tier-1 data is told so instead of receiving an implied damage conclusion. | `src/geosteward/agents/decision.py` |
| distribution plane | 发布平面 | The half of `policy_v1.yaml` scoping what a build may publish, over each artifact's `resolution_cap`, `audience`, and `license`; CI checks the assembled site against it by set difference and fails the deploy on a violation. Added after the 2026-08-20 incident, alongside the claim plane rather than folded into it. | `src/geosteward/harness/distribution.py` |
| fail-closed | 失败即拒绝（fail-closed） | The default a stage takes when nothing authorizes it or a check does not pass: refuse, and record the reason — never guess, substitute a cache, or wave a request through because no rule happened to deny it. | `src/geosteward/harness/policy.py` |
| H3 r9 | H3 r9（分辨率 9 级网格） | The H3 hexagonal grid at resolution 9, roughly 0.1 km² per cell — the finest geography the project's evidence can support a public statement about, and the resolution cap tile-level artifacts share. | `src/geosteward/gateway/steward.py` |
| NIFC/WFIGS | NIFC/WFIGS | The US National Interagency Fire Center's Wildland Fire Interagency Geospatial Services feed: a keyless, current-incident wildfire source feeding the nationwide Tier-1 watch layer. | `src/geosteward/sources/nifc.py` |
...
| Steward Harness | Steward Harness（问责框架） | The accountable layer wrapped around the agent: executable outcome checks, append-only process audit, and a declarative institutional policy engine, standing on both sides of the model so the LLM only drafts prose from evidence it was handed. | `src/geosteward/harness/__init__.py` |
