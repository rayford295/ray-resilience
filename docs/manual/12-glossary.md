...
| resolved unknowns | 已解除的未知项 | A dossier's list of declared unknowns that a later, *registered* artifact has made untrue. Each entry keeps the retired sentence plus the resolver's path, sha256, time and stage — the unknown is moved, never deleted, and the dossier is reissued through the harness so the manifest keeps both versions. Introduced 2026-09-03 (`src/geosteward/deepcase/dossier.py`). |
