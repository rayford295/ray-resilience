...
| `scripts/build_palisades_vlm.py` | Builds `events/palisades-2025/` (an *evaluation* case, not in `published_events`): grades RAPID's 295-image LA DINS street-view set with whatever OpenAI-compatible vision endpoint `STEWARD_LLM_BASE_URL` names (local Ollama by default), writes per-image records (lineage-only: they carry EXIF coordinates), an accuracy/NCSE/confusion summary that quotes the paper's closed-model numbers beside it, a tile grid of agreement, and the dossier. Resumable by image sha256; aborts if more than 20 % of answers are off-schema. `--run-tag <tag>` (or `auto`, the served model's slug) suffixes every output of the run — records, grid, eval — so several models' runs sit beside the untagged reference run instead of overwriting it. | `src/geosteward/deepcase/vlm_severity.py`, `src/geosteward/gateway/llm.py`, `src/geosteward/harness/checks/outcome.py`, `src/geosteward/harness/audit.py` | no direct test — needs a running vision model; the logic it composes is tested in `tests/test_deepcase_vlm_severity.py` |
| `scripts/build_milton_vlm_bitemporal.py` | Adds a zero-shot pre/post pair grading stage to the Milton case from the public Bi-Temporal set: images from the Hugging Face release, pair ids / coordinates / human labels from the Figshare pair table (the same table the committed `bitemporal_h3_r9_grid.geojson` was built from). RAPID's pre/post prompt verbatim; seeded stratified `--sample` per class; fails closed unless ≥ 95 % of graded pairs fall inside the committed grid's cells. Writes the registry profile, a coordinates-bearing pair-table snapshot (internal), the prompt, lineage-only pair records, an accuracy/NCSE summary and a per-tile agreement grid. `--run-tag <tag>` (or `auto`, the served model's slug) suffixes every output of the run — records, grid, eval — so several models' runs sit beside the untagged reference run instead of overwriting it. | `src/geosteward/deepcase/vlm_severity.py`, `src/geosteward/gateway/llm.py`, `src/geosteward/harness/checks/outcome.py`, `src/geosteward/harness/audit.py`, `h3` | no direct test — needs a running vision model; pair parsing and record shape are tested in `tests/test_deepcase_vlm_severity.py` |
| `scripts/run_vlm_sweep.py` | Runs the three zero-shot VLM builders for several served Ollama models in turn, each with `--run-tag auto` so the outputs sit beside the reference run; records per (model, case) the exit status, elapsed time, the eval summary, and how the server placed the model (`/api/ps`: bytes in VRAM vs total, context length) — a model spilled to CPU is a different run and the summary says so. Skips models that are not served (never pulls), continues past a failed case, unloads a model before the next one, is resumable through the builders' own sha256 / pair-id skip, and regenerates `docs/vlm_model_comparison.md` at the end. | `scripts/build_*_vlm*.py`, `src/geosteward/deepcase/vlm_severity.py` (`model_slug`, `tagged_path`) | `tests/test_deepcase_vlm_sweep.py` (planning, tagged paths, unreachable server — no model) |
| `scripts/compare_vlm_models.py` | Renders `docs/vlm_model_comparison.md` (and JSON) from every `vlm_*_eval[.tag].json` under `events/*/evidence/`: one block per (event, task), one row per model run with accuracy / NCSE / adjacent-error / per-class recall, the paper's closed-model numbers as reference rows where the eval file cites them, and a `same setup` flag for rows whose prompt sha256 or seeded sample differs from the reference run. Copies numbers; recomputes nothing. | `src/geosteward/agents/base.py` (`utc_stamp`) | `tests/test_deepcase_vlm_sweep.py` |
