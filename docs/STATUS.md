...
| 6 | ~~**Repository visibility decision**~~ **RESOLVED 2026-09-04 21:10 CDT — repository is public** (`gh repo edit --visibility public`; anonymous fetch of the `live-data` branch verified, so the app's Tier-1 watch layer now loads) | Judge access + PWA data path | **Owner decision 2026-09-04: the repository goes public for the submission.** Before the flip the tracked tree was swept for workstation paths (builder defaults now read `DISASTER_DATASET_ROOT`; manifest input paths under the maintainer's dataset roots were rewritten to `<DATASET_ROOT>` / `<RAPID_ROOT>` placeholders — the sha256 in each row stays the verifiable anchor; test fixtures use a neutral username). Nothing else in the tree is a secret: the gateway is keyless, the app is keyless, and every dataset snapshot committed is public or CC-licensed. **Resolved 2026-09-04 22:00 CDT:** `dins_points_restricted.csv.gz` (Eaton exposure directory; removed from the public tree 2026-09-04) (parcel-resolution DINS points) was removed from the tracked tree — a public repository is itself a distribution channel, and the distribution plane denies parcel resolution; keeping the file public while calling it restricted was the same contradiction as the 2026-08-20 incident. The manifest row (kind, sha256, inputs) stays as the record that the artifact existed; the file is retained privately by the maintainer and is regenerated by `build_eaton_case.py` from the registry. It remains in git history before this commit; purging history is the owner's call (it invalidates every clone). |
