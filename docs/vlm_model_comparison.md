...
Generated 20260903T202612Z by `scripts/compare_vlm_models.py` from the committed eval summaries under `events/*/evidence/`. Every number is copied from a `vlm_eval_summary` file that passed its builder's fail-closed checks; nothing is recomputed here.
