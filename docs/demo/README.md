...
| Pipeline, end to end | a real `scripts/judge_demo.py` transcript replayed line by line: four federal feeds fetched and snapshotted, the national watch product built with audit rows, the distribution plane's allowed/denied counts by rule, and the claim plane's decisions for a role × purpose × resolution matrix | `docs/demo/out/judge_demo.txt`, captured moments before recording (`python scripts/judge_demo.py --out judge-out > docs/demo/out/judge_demo.txt`) |
