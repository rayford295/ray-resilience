...
| Pipeline, end to end | a real `scripts/judge_demo.py` transcript replayed line by line: four federal feeds fetched and snapshotted, the national watch product built with audit rows, the distribution plane's allowed/denied counts by rule, and the claim plane's decisions for a role × purpose × resolution matrix | a transcript file named `judge_demo.txt` in the git-ignored `out/` directory, captured moments before recording by redirecting `python scripts/judge_demo.py --out judge-out` into it |
